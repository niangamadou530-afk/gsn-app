...
export default function MctnCourseDetailPage() {
  ...
  async function load() {
    ...
    if (error || !data) { router.replace("/mctn/learn"); return; }
  ...
  const loadContent = useCallback(async (id: string, m: Module, w: Week) => {
    ...
    try {
      const domain = (course?.title ?? "Formation").split("·")[1]?.trim() ?? "Formation";
  ...
  async function submitExercise(id: string) {
    ...
    if (error) console.error("submitExercise error:", error.message);
    ...
    supabase.from("user_courses").update({ modules_progress: updatedProgress })
      .eq("id", courseId).eq("user_id", userId)
      .then(({ error: e }) => { if (e) console.warn("modules_progress not saved:", e.message); });
  }
  ...
  async function submitModQuiz(id: string, quiz: QuizQ[]) {
    ...
    if (passed) {
      ...
      if (error) console.error("submitModQuiz error:", error.message);
  ...
  // Title display: "PFIMN · Développement Web & Mobile · …" → show middle part
  const titleParts = course.title.split("·").map((p) => p.trim());
  const shortTitle = titleParts[1] ?? titleParts[0];

  return (
    <main className="min-h-screen bg-surface text-on-surface pb-32">

      {/* Top bar */}
      <header className="fixed top-0 w-full z-50 bg-[#0a1628]/95 backdrop-blur-md shadow-sm flex justify-between items-center px-6 py-4">
        <Link
          href="/mctn/learn"
        ...
        <span className="text-sm font-bold text-white/90 truncate max-w-[160px]">{shortTitle}</span>
        <div className="flex items-center gap-2">
          <span className="text-xs font-bold text-white/50 uppercase tracking-widest">PFIMN</span>
      ...
      <div className="pt-24 px-6 max-w-2xl mx-auto space-y-6">
        ...
        <div className="space-y-3">
          <div className="inline-flex items-center gap-2 bg-primary/10 text-primary text-[10px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-full">
                          ...
                          <div className="flex-1">
                            ...
                            {!accessible && (
                              <p className="text-xs text-on-surface-variant mt-0.5">
                                Valide le module précédent pour débloquer
                              </p>
                            )}
                            {isPassed && (
                              <p className="text-xs text-emerald-600 mt-0.5 font-medium">Quiz validé ✓</p>
                            )}
                          </div>
                          {accessible && (
                                      ...
                                      <div className="space-y-3">
                                        {yt[id].map((v, vi) => (
                                          <div key={vi} className="rounded-xl overflow-hidden bg-black">
                                            ...
                                            <p className="text-xs text-on-surface-variant px-3 py-2 bg-surface-container-low">
                                              {v.title}
                                            </p>
                                          </div>
                                        ))}
                            ...
                            {phase === "exercise" && (
                              <div className="space-y-4">
                                <div className="bg-tertiary-fixed/30 rounded-xl p-4">
                                  <p className="text-[10px] font-bold text-tertiary uppercase tracking-widest mb-2">
                                    Exercice pratique
                                  </p>
                                  <p className="text-sm text-on-surface font-medium leading-relaxed">
                                    {m.exercises ||
                                      "Applique les concepts du module dans un exercice pratique. Décris ta démarche, les étapes que tu as suivies et ce que tu as appris."}
                                  </p>
                                </div>
                                <div>
                                  ...
                                  <textarea
                                    value={exText[id] ?? ""}
                                    onChange={(e) =>
                                      setExText((prev) => ({ ...prev, [id]: e.target.value }))
                                    }
                                    rows={5}
                            ...
                            {phase === "quiz" &&
                              ...
                              (() => {
                                ...
                                const correct = quiz.filter(
                                  (qq, i) => ans[i] === qq.answer
                                ).length;
                                const scorePct = Math.round((correct / totalQ) * 100);
                                const quizPassedLocal = scorePct >= 60;
                                ...
                                return (
                                  <div className="space-y-4">
                                    ...
                                    <div className="bg-surface-container-low rounded-xl p-4">
                                      ...
                                      <div className="space-y-2">
                                        {q.options.map((opt, oi) => {
                                          let cls =
                                            "w-full text-left px-3 py-2.5 rounded-xl border-l-4 text-sm transition-colors ";
                                          if (!isSubmitted) {
                                            cls +=
                                              userAns === oi
                                          ...
                                          return (
                                            <button
                                              key={oi}
                                              className={cls}
                                              onClick={() => pickAnswer(id, curQIdx, oi)}
                                      ...
                                      {isSubmitted && quizPassedLocal && q.explanation && (
                                    ...
                                    {!isSubmitted && (
                                      <div className="flex gap-2">
                                        ...
                                        {curQIdx < totalQ - 1 ? (
                                          <button
                                            onClick={() =>
                                              setQIdx((prev) => ({ ...prev, [id]: curQIdx + 1 }))
                                            }
                                    ...
                                    {isSubmitted && (
                                      <div
                                        className={`rounded-xl p-4 text-center ${quizPassedLocal ? "bg-emerald-50 border border-emerald-200" : "bg-red-50 border border-red-200"}`}
                                      >
                                        <p
                                          className={`text-2xl font-black mb-1 ${quizPassedLocal ? "text-emerald-600" : "text-red-500"}`}
                                        >
                                          {scorePct}%
                                        </p>
                                        <p className="font-semibold text-sm mb-1">
                                          {quizPassedLocal ? "Quiz réussi ✓" : "Quiz échoué ✗"}
                                        </p>
                                        <p className="text-xs text-on-surface-variant mb-3">
                                          {correct}/{totalQ} bonnes réponses ·{" "}
                                          {quizPassedLocal
                              ...
                              <div className="space-y-4">
                                <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-5 text-center">
                                  ...
                                  <p className="text-xs text-on-surface-variant mt-1">
                                    Quiz réussi · Module suivant débloqué
                                  </p>
                                  <button
                                    onClick={() => retryModQuiz(id)}
                                    className="mt-3 text-xs text-on-surface-variant hover:text-on-surface underline"
                                  >
                                    Repasser le quiz
                                  </button>
                                </div>
        ...
        {/* ── Test final PFIMN ── */}
        <div className="bg-gradient-to-br from-[#0a1628] to-[#0d2050] rounded-2xl p-6 shadow-lg text-center space-y-3">
          <p className="text-[10px] font-bold text-primary uppercase tracking-widest">
            Certification PFIMN
          </p>
          <h3 className="font-extrabold text-lg text-white">Test final certifié</h3>
          <p className="text-sm text-white/60">
            20 questions · Score minimum 70% · Skill Passport NDT
          </p>
          {passedCount < totalMods && totalMods > 0 && (
            <div className="inline-flex items-center gap-2 bg-white/10 text-white/60 rounded-xl px-4 py-2 text-xs font-bold">
