...
export default function MctnCertificatePage() {
  ...
  async function load() {
    ...
    if (courseRes.error || !courseRes.data) { router.replace(`/mctn/learn/${courseId}/test`); return; }
    if (!courseRes.data.certificate_id) { router.replace(`/mctn/learn/${courseId}/test`); return; }
  ...
  const domain = (course.title ?? "Formation PFIMN").split("—")[0].trim();
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-surface text-on-surface flex flex-col">
        ...
        <header className="no-print fixed top-0 w-full z-50 bg-[#0a1628]/95 backdrop-blur-md flex justify-between items-center px-6 py-4 shadow-md">
          <Link href={`/mctn/learn/${courseId}`} className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-white/10 active:scale-95 transition-all">
            <span className="material-symbols-outlined text-white">arrow_back</span>
          </Link>
          <span className="text-base font-bold text-white">Certificat PFIMN</span>
          <div className="w-10" />
        </header>
        ...
        <div className="flex-grow pt-24 pb-32 px-4 max-w-3xl mx-auto w-full">
          ...
          <div className="no-print mb-8 text-center">
            ...
            <p className="text-on-surface-variant">Votre excellence a été reconnue et certifiée — New Deal Technologique.</p>
          </div>
          ...
          <div className="cert-wrap relative bg-surface-container-lowest rounded-2xl shadow-xl shadow-on-surface/5 p-1 overflow-hidden border-t-4 border-primary">
            ...
            <div className="relative border-[12px] border-surface-container-high m-4 p-8 md:p-12 flex flex-col items-center text-center">
              ...
              <div className="mb-8 flex flex-col items-center">
                ...
                <span className="text-on-surface-variant text-xs tracking-[0.3em] font-semibold uppercase">PFIMN · MCTN · NDT Sénégal</span>
              </div>
              ...
              <div className="flex flex-col items-center">
                <div className="w-40 h-10 border-b border-on-surface-variant/30 mb-2 flex items-center justify-center">
                  <span className="font-serif italic text-xl text-on-surface-variant/60">MCTN · GSN</span>
                </div>
                <p className="text-on-surface-variant font-bold text-[10px] uppercase tracking-widest">PFIMN · New Deal Technologique</p>
              </div>
          ...
          <div className="no-print mt-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              ...
              {/* WhatsApp */}
              <a
                href={`https://wa.me/?text=${encodeURIComponent(`Je viens d'obtenir ma certification ${domain} via le PFIMN (New Deal Technologique - Sénégal) ! Score : ${course.test_score}% 🎓 #PFIMN #MCTN #NDT\n${typeof window !== "undefined" ? window.location.href : ""}`)}`}
                target="_blank" rel="noopener noreferrer"
              ...
              {/* X / Twitter */}
              <a
                href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(`Je viens d'obtenir ma certification ${domain} via le PFIMN (New Deal Technologique - Sénégal) ! Score : ${course.test_score}% 🎓 #PFIMN #MCTN #NDT`)}&url=${encodeURIComponent(typeof window !== "undefined" ? window.location.href : "")}`}
                target="_blank" rel="noopener noreferrer"
        ...
        <nav className="no-print fixed bottom-0 left-0 w-full z-50 glass-nav rounded-t-3xl shadow-[0_-4px_24px_rgba(25,28,35,0.06)] flex justify-around items-center px-4 pb-6 pt-3">
          ...
          <Link href="/mctn/passport" className="flex flex-col items-center text-primary relative after:content-[''] after:absolute after:-bottom-1 after:w-1 after:h-1 after:bg-primary after:rounded-full active:scale-90 transition-transform">
