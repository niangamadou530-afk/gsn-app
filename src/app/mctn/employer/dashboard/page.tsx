...
type Employer = { id: string; company_name: string; email: string; tenant_id: string };
type Mission = {
  id: string;
  title: string;
  domain: string;
...
export default function MctnEmployerDashboard() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!emp || emp.tenant_id !== "mctn") { router.replace("/mctn/employer/login"); return; }
      setEmployer(emp);
  ...
  return (
    <main className="min-h-screen bg-surface text-on-surface pb-8">
      {/* Top nav */}
      <header className="sticky top-0 z-30 bg-[#0a1628]/95 backdrop-blur-md px-6 py-4 flex items-center justify-between shadow-md">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-primary rounded-lg flex items-center justify-center">
            <span className="text-white font-black text-xs">GSN</span>
          </div>
          <div>
            <p className="text-xs text-white/50 font-medium uppercase tracking-widest">Espace Entreprises PFIMN</p>
            <p className="text-sm font-bold text-white leading-tight">{employer?.company_name}</p>
          </div>
        </div>
        <button onClick={handleSignout} className="flex items-center gap-1.5 text-sm text-white/60 hover:text-white transition-colors font-medium">
      ...
      <div className="max-w-4xl mx-auto px-6 py-8 space-y-8">
        ...
        {/* Welcome */}
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight text-on-surface">Tableau de bord NDT</h1>
          <p className="text-on-surface-variant text-sm mt-1">Publiez des offres et recrutez les talents certifiés PFIMN</p>
        </div>

        {/* Stats */}
        <div className="grid grid-cols-3 gap-4">
          {[
            { icon: "work", label: "Offres publiées", value: missions.length, color: "text-primary" },
            { icon: "check_circle", label: "Offres actives", value: activeMissions.length, color: "text-tertiary" },
        ...
        <section className="space-y-4">
          <h2 className="text-lg font-bold text-on-surface">Mes offres</h2>
          ...
          {missions.length === 0 ? (
            <div className="bg-surface-container-lowest rounded-2xl p-10 text-center shadow-[0_4px_16px_rgba(25,28,35,0.06)]">
              ...
              <p className="text-on-surface-variant text-sm font-medium">Aucune offre publiée pour l&apos;instant.</p>
              <Link href="/mctn/employer/missions/new" className="inline-block mt-4 text-primary font-bold text-sm hover:underline">
            ...
            <div className="space-y-3">
              {missions.map(m => (
                <Link key={m.id} href={`/mctn/employer/missions/${m.id}`}
