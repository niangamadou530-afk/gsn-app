...
export default function MctnEmployerLoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      if (!emp) {
        ...
        if (empByEmail) {
          await supabase.from("employers").update({ auth_id: authData.user.id }).eq("email", authData.user.email);
          emp = empByEmail;
        }
      }

      if (!emp) { setError("Aucun compte employeur PFIMN associé à cet email."); return; }
      if (emp.tenant_id !== "mctn") { setError("Ce compte n'est pas un compte employeur PFIMN / MCTN."); return; }
    ...
    <main className="min-h-screen bg-surface text-on-surface flex flex-col items-center justify-center p-6">
      ...
      <div className="w-full max-w-md flex flex-col items-center space-y-8">
        ...
        <header className="flex flex-col items-center space-y-3 text-center">
          <div className="w-16 h-16 bg-[#0a1628] rounded-2xl flex items-center justify-center shadow-lg mb-1">
            <span className="material-symbols-outlined text-white text-3xl" style={{ fontVariationSettings: "'FILL' 1" }}>business</span>
          </div>
          <h1 className="text-3xl font-extrabold tracking-tight text-on-surface">Espace Entreprises PFIMN</h1>
          <p className="text-on-surface-variant text-sm">MCTN · New Deal Technologique</p>
        </header>
        ...
        <section className="w-full bg-surface-container-lowest rounded-2xl p-8 shadow-[0_8px_24px_rgba(25,28,35,0.07)] space-y-6">
          ...
          <Link href="/mctn/employer/signup"
            ...
            Créer un compte entreprise PFIMN
          </Link>
        </section>

        <footer className="flex items-center justify-center space-x-6 text-outline font-medium text-xs">
          <Link href="/mctn" className="hover:text-on-surface transition-colors">Espace PFIMN</Link>
