...
export default function MctnMissionCandidatesPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: m, error: mErr } = await supabase
        ...
        .single();
      if (mErr || !m) { router.replace("/mctn/employer/dashboard"); return; }
      setMission(m);
    ...
    <main className="min-h-screen bg-surface text-on-surface">
      <header className="sticky top-0 z-30 bg-[#0a1628]/95 backdrop-blur-md px-6 py-4 flex items-center gap-3 shadow-md">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-xs text-white/50 font-medium">Offre PFIMN</p>
          <p className="font-bold text-white truncate">{mission?.title}</p>
        </div>
        <button
                ...
                <div key={c.id} className="bg-surface-container-lowest rounded-2xl p-5 shadow-[0_4px_16px_rgba(25,28,35,0.06)] space-y-4">
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-bold text-on-surface">{c.user?.name ?? "Talent PFIMN"}</p>
