...
const DOMAINS = [
  ...
  "Data Science & IA",
  "Cybersécurité & Sys. Admin",
  "UX Design & Contenu Digital",
  "E-commerce & Marketing",
  "Cloud Computing & DevOps",
  "Autre",
];

export default function NewMctnMissionPage() {
  ...
  useEffect(() => {
    async function checkAuth() {
      ...
      const { data: emps } = await supabase.from("employers").select("id, tenant_id").eq("auth_id", auth.user.id).limit(1);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      router.push("/mctn/employer/dashboard");
    } catch {
  ...
  return (
    <main className="min-h-screen bg-surface text-on-surface">
      ...
      <header className="sticky top-0 z-30 bg-[#0a1628]/95 backdrop-blur-md px-6 py-4 flex items-center gap-3 shadow-md">
        <Link href="/mctn/employer/dashboard" className="text-white/60 hover:text-white transition-colors">
          <span className="material-symbols-outlined text-[22px]">arrow_back</span>
        </Link>
        <h1 className="font-bold text-white text-lg">Publier une offre NDT</h1>
      </header>
      ...
      <div className="max-w-2xl mx-auto px-6 py-8">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            <label className="block text-sm font-semibold text-on-surface ml-1">Titre de l&apos;offre *</label>
          ...
          {/* Domain */}
          <div className="space-y-2">
            <label className="block text-sm font-semibold text-on-surface ml-1">Domaine NDT *</label>
            <div className="relative group">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-on-surface-variant ml-1">Seuls les talents PFIMN ayant ce score ou plus verront cette offre.</p>
          </div>
          ...
          <button type="submit" disabled={loading}
            ...
            {loading ? (
              ...
              <><span className="material-symbols-outlined text-[20px]" style={{ fontVariationSettings: "'FILL' 1" }}>publish</span> Publier l&apos;offre</>
            )}
          </button>
        </form>
      </div>
