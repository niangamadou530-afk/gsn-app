  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: stu } = await supabase
        .from("prep_students").select("serie, ecole").eq("user_id", user.id).maybeSingle();
      ...
      const { data: students } = await supabase
        .from("prep_students").select("user_id, prenom, ecole, serie").in("user_id", uids);

      const stuMap: Record<string, { prenom: string | null; ecole: string | null; serie: string | null }> = {};
      for (const s of students ?? []) stuMap[s.user_id] = { prenom: s.prenom, ecole: s.ecole, serie: s.serie };

      const players: Player[] = uids.map(uid => ({
        user_id: uid,
